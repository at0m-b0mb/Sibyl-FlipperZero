...
The classifier, the feature extractor and the reference library are pure integer logic
with no radio, no GUI and no floats, so they run on the host under ASan and UBSan:

```bash
make -C test
```

That is ~4,800 checks over synthetic pulse trains built to look like the real thing —
including the cases where a naive estimator goes wrong. The three that earn their keep:
